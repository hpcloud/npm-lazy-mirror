...
config.getProxyConfig = function(cb) {
    Npm.load(function(err, npm) {
        if (err) { return (err); }
        return cb(null, {
            http_proxy: Argv.http_proxy || Config.http_proxy || Npm.config.get('proxy') || process.env.HTTP_PROXY,
            https_proxy: Argv.https_proxy || Config.https_proxy || Npm.config.get('https-proxy') || process.env.HTTPS_PROXY
        });
    });
};
