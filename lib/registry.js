...
Registry.prototype.proxyUpstream = function (req, res) {
    ...
    url.protocol = proto;
    url.port = port;
    url.pathname = registry.config.upstreamDBPath + url.pathname;
...
Registry.prototype.getTarball = function (pkg, cb) {
    ...
    Async.series([
        ...
        function (done) {
            var requestOpts = {
                url: proto + '://' + pkg.config.upstreamHost + ':' + pkg.config.upstreamPort + pkg.config.upstreamDBPath + '/' + pkg.name + '/-/' + pkg.name + '-' + pkg.version + '.tgz',
